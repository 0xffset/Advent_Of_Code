//
// Created by u-0xffset on 04.12.20.
//
...
static std::vector<std::vector<std::string>> in = {
...
int y2020::day4part1() {
    ...
    bool foundCID;
    for (const std::vector<std::string> &v : in) {
        foundCID = false;
        if (v.size() == 8) {
            validPassports++;
            continue;
...
static std::map<std::string, int> attributeTable = {
        {"byr", 0},
        {"iyr", 1},
        {"eyr", 2},
        {"hgt", 3},
...
int y2020::day4part2() {
    ...
    for (const std::vector<std::string> &v : in) {
        ...
        for (const std::string &s : v) {
            switch (attributeTable.at(s.substr(0, s.find(':')))) {
                case 0:
                    temp = s.substr(s.find(':')+1);
                    if (temp.size() != 4 || std::stoi(temp) > 2002 || std::stoi(temp) < 1920) {
                        invalid = true;
                    }
                    break;
                ...
                case 1:
                    temp = s.substr(s.find(':')+1);
                    if (temp.size() != 4 || std::stoi(temp) > 2020 || std::stoi(temp) < 2010) {
                        invalid = true;
                    }
                    break;
                ...
                case 2:
                    temp = s.substr(s.find(':')+1);
                    if (temp.size() != 4 || std::stoi(temp) > 2030 || std::stoi(temp) < 2020) {
                        invalid = true;
                    }
                    break;

                case 3:
                    temp = s.substr(s.find(':')+1, s.size()-2);
                    if (s.at(s.size()-1) == 'm') { // if cm
                        if (std::stoi(temp) > 193 || std::stoi(temp) < 150) {
                            invalid = true;
                        }
                    } else if (s.at(s.size()-1) == 'n'){
                        if (std::stoi(temp) > 76 || std::stoi(temp) < 59) {
                            invalid = true;
                        }
                    } else {
                        invalid = true;
                ...
                case 4:
                    temp = s.substr(s.find(':')+2);
                    if (s.at(s.find(':')+1) != '#' || temp.size() != 6 || !isHex(temp)) {
                        invalid = true;
                    }
                    break;
                ...
                case 5:
                    temp = s.substr(s.find(':')+1);
                    if (std::find(eyeColors.begin(), eyeColors.end(), temp) == eyeColors.end()) {
                        invalid = true;
                    }
                    break;
